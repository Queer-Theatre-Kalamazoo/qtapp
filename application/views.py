    ...
    with Session.begin() as session:
        with Session.begin() as session:
            staff = session.execute(
                select(Person.name, Person.person_id, Person.headshot, Person.biography, Person.slug, Relationship.title).select_from(Person).where(and_(Relationship.type == "Staff", Relationship.show_online == True)).join(Relationship, Relationship.person_id == Person.person_id)).all()
            board = session.execute(
                select(Person.name, Person.person_id, Person.headshot, Person.biography, Person.slug, Relationship.title).select_from(Person).where(and_(Relationship.type == "Board", Relationship.show_online == True)).join(Relationship, Relationship.person_id == Person.person_id)).all()
